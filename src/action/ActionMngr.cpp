...
void ActionMngr::notifyActionBefore(CommandType type, const OperandList &operands, Context &context){
    do_notifyAction(m_beforeActions, type, operands, context);
...
void ActionMngr::notifyActionAfter(CommandType type, const OperandList &operands, Context &context){
    do_notifyAction(m_afterActions, type, operands, context);
 ...
 *   0 --- Action为全局action，即无论scopeid是多少，都会尝试执行该Action
 */
...
void ActionMngr::do_notifyAction(ScopedActionMap &map, CommandType type, const OperandList &operands, Context &context){
    /* 根据Scope的id找到对应的Action队列*/
    auto actions = map.find(context.scopeMetaData().id);
    if(actions == map.end()){
        return;
    }

    /* 依次匹配每个队列中的Action，并尝试执行 */
